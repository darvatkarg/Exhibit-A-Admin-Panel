...
const columns = [
  ...
  },
  {
    title: "School",
    dataIndex: "userDetail",
    render: (text) => <a>{text[0]?.schoolName}</a>,
  },
  {
    title: "Email",
    dataIndex: "userDetail",
...
const Betch_managment = () => {
  ...
  const submit = () => {
    ...
    if (subject && dates && addTime && user.length) {
      const body = {
        ...
        time_slotId: addTime,
        subjectId: subject,
        selectedUser: user.map((val) => val._id),
        // selectedUserEmails: user.map(val => val.email),
      };
  ...
  const rowSelection = {
    onSelectAll: (selected, selectedRows, changeRows) => {
      const selectedId = selectedRows.map((item) => item.userDetail[0]);
                    ...
                    dataSource={items
                      .filter((val) => {
                        ...
                        if (rigion == "" || rigion == undefined) {
                          ...
                          val.userDetail[0]?.region
                            ?.toLowerCase()
                            .includes(rigion?.toLowerCase())
                        ) {
                          // if (
                          //   val.userDetail[0]?.email?.split("@")[
                          //     val.userDetail[0]?.email?.split("@").length - 1
                          //   ] == "ges.gov.gh"
                          // ) {
                            return val;
                          // }
                      ...
                      .filter((vals) => {
                        if (values2 === "") {
                          ...
                          vals.userDetail[0]?.schoolName
                            ?.toLowerCase()
                            .includes(values2?.toLowerCase())
                        ) {
                          return vals;
                        }
                      })}
