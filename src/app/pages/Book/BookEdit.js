...
export default function CreateSubCategory() {
  ...
  const fetchData = async (id) => {
    ...
    ApiGet("/book/" + id)
      .then((res) => {
        ...
        if (res.data.data.isPhysical == true) {
          res.data.data.isPhysical = "true";
        } else if (res.data.data.isPhysical == false) {
          res.data.data.isPhysical = "false";
        }
  ...
  const validateForm = () => {
    ...
    if (!accountData.subCategoryId) {
      console.log("c");
      formIsValid = false;
      errors["subCategoryId"] = "*Please enter sub category";
    }
  ...
  const onSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        if (accountData.isPhysical == "true") {
          isPhy = true;
        } else {
          isPhy = false;
        }
        let body = {
          ...
          publisher: accountData.publisher,
          isFree: isFREE,
          isPhysical: isPhy,
          image: image[0],
                ...
                <Row>
                  <Col md={6}>
                    <Row>
                      ...
                      <Col md={8}>
                        <Label>
                          Is physical delivery available?<span style={{ color: "red" }}> * </span>
                        </Label>
                        <RadioGroup
                          aria-label="isPhysical"
                          name="isPhysical"
                          value={accountData.isPhysical}
                          onChange={(e) => handleonChange(e)}
                        >
                          <Row>
                            <Col md="6">
                              <FormControlLabel
                                ...
                                control={<Radio />}
                                label="Yes"
                                checked={accountData.isPhysical === "true"}
                              />
                            </Col>
                            <Col md="6">
                              <FormControlLabel
                                ...
                                control={<Radio />}
                                label="No"
                                checked={accountData.isPhysical === "false"}
                              />
                            </Col>
                          </Row>
                        </RadioGroup>
