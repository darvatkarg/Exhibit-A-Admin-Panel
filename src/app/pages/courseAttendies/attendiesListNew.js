...
import Upload_Doc_Modal from "./Upload_Doc_Modal";
const spacing = {
  margin: "1%",
};
...
export default function SellerDashboard(props) {
      ...
      Cell: (row) => {
        ...
        return (
          <div>
            ...
            <div className="cursor-pointer" onClick={() => openModal(row.original)}><BsFileEarmarkPlusFill fontSize={30} color='#3699FF' /></div>


          </div>
          // <div className="text-center">
          //   {row?.original?.document_image === null ? (
          //     <div>No Document Upload</div>
          //   ) : row.original?.document_image?.includes("pdf") ? (
          //     <a href={Bucket + row?.original?.document_image} target="_blank">
  ...
  const getDate = async (s, p, search, subjectId) => {
    ...
    await ApiPost("/student/student_not_upload_form", body)
      .then((res) => {
        console.log("res", res);
  ...
  const exportSheet = async () => {
    ...
    console.log("getDdata", body);
    const Id2 = JSON.parse(localStorage.getItem("token"));
    ApiPost("/student/student_not_upload_form", body)
      .then((res) => {
        setCategory(res?.data?.data?.student_data);
                    ...
                    <FormGroup className="d-flex align-items-end ">
                      <div className="w-100">
                        ...
                        <Input
                          ...
                          placeholder="Enter Name email  or teacher Id"
                          // required
                        />
                      </div>
                      <button
