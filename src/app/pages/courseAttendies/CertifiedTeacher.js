...
let search = "";
export default function CertifiedTeacher(props) {
  ...
  const uploadFiles = () => {
    ...
    ApiUpload('upload/file_upload/form_image', formData).then(res => console.log('image', res)).catch(err => console.log('err', err))
  ...
  const [columns, setcolumn] = useState([
        ...
        return (
          <div>
            {row.original.isDocument === true ? (
              ...
              ) : row.original.documents[0]?.document_image?.toLowerCase()?.includes("pdf") ? (
                <a
              ...
              ) : row.original.documents[0]?.document_image?.toLowerCase()?.includes("docx") ? (
                <a
                  href={Bucket + row?.original?.documents[0]?.document_image}
                  target="_blank"
                >
    ...
    // },
    {
      ...
      Cell: (row) => {
        console.log('date.........',row.original?.createdAt.split("T")[0] )
        return (
          <div>
            <center>
              {row.original.createdAt &&
                moment(row.original?.createdAt.split("T")[0]).format("DD-MM-YYYY")}
            </center>
          </div>
        );
      },
  ...
  const getDate = (s, p, search, subjectId) => {
    ...
    ApiPost("/student/result_status/approve", body)
      .then((res) => {
        console.log("res", res);
  ...
  const dateChange = async (date,s, p, search, subjectId)=>{
    setloading(true)
    SetPage(1)
    ...
    console.log('body', body)
    await ApiPost("/student/result_status_approve/filter_date",body)
    .then(res => {console.log('res', res.data.data); setRecordCount(res?.data?.data);
    setCategory(res?.data?.data?.student_data);
    SetPage(res?.data?.data?.state?.page);
    SetPages(res?.data?.data?.state?.page_limit);
    SetPageSize(res?.data?.data?.state?.limit)
    ...
    <Container style={position}>
      ...
      <Row>
        <Col md={12}>
          <ToastContainer />
          <Breadcrumb>
            <BreadcrumbItem active>List of All Certified Teacher</BreadcrumbItem>
          </Breadcrumb>
        </Col>
          ...
          <Card>
            <CardBody>
              ...
              <div className="row">
                ...
                <Col>
                  <button className="btn btn-primary d-flex align-items-center" disabled={isLoading} onClick={exportSheet}>Export ExcelSheet</button>
