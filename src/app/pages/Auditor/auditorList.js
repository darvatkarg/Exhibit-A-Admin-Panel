...
export default function SellerDashboard(props) {
  ...
  const EditQuestion = (row) => {
    history.push("/auditorEdit?id=" + row);
  };
  ...
  const BlockPage = (row) => {
    console.log("/auditor/delete/" + row);
    ApiDelete("/auditor/delete/" + row)
      .then((res) => {
        if (res.status === 200) {
          handleClose1();
  ...
  const getDate = (s, p, q) => {
    ...
    ApiPost("/auditor/get_auditor", body)
      .then((res) => {
        setCategory(res.data.data.auditor_data);
      ...
      <Row>
        <Col md={12}>
          ...
          <Breadcrumb className="breNav">
            <BreadcrumbItem active>List of all auditor</BreadcrumbItem>
            <button
              id="kt_login_signin_submit"
              type="submit"
              className={`btn btn-primary font-weight-bold px-8 py-2`}
              // onClick={() => history.push("/genreEdit")}
              onClick={() => history.push("/auditorEdit")}
            >
              <span>Add</span>
            </button>
          </Breadcrumb>
